...
export default async function Home() {
  // Получаем сессию пользователя на сервере (заменили старый вызов auth())
  ...
  return (
    <HydrateClient>
      <div className="flex flex-col items-center justify-center min-h-[70vh] py-10 text-center space-y-8 max-w-3xl mx-auto px-4">
        ...
        {/* Информационная плашка статуса пользователя */}
        <div className="p-4 rounded-xl border bg-card w-full max-w-md text-sm">
          {session ? (
            <p className="text-muted-foreground">
              ...
              {session.user?.role === "ADMIN" && <span className="ml-2 px-1.5 py-0.5 text-[10px] bg-red-500/10 text-red-500 font-bold rounded">ADMIN</span>}
            </p>
          ) : (
            <p className="text-muted-foreground">
        ...
        {/* Быстрые переходы по главным разделам PWA */}
